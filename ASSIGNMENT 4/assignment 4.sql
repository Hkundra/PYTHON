...
14.CREATE TABLE STUDENT( stud_name VARCHAR(20),stud_id INT PRIMARY KEY,dob DATE,addr VARCHAR(20),age INT);
...
18.SELECT * FROM Emp_Table
     ORDER BY Ename DESC;
19.CREATE TABLE EMP_BKP(EmpNO INT PRIMARY KEY,Ename VARCHAR(10),SAL INT,HIRE_Dtae date,Commision INT,DeptNo INT,Mrg INT);
...
25.SELECT Mgr ,Ename
FROM Emp_Table
GROUP BY Mgr HAVING COUNT(*)=(SELECT MAX(COUNT(MGR)) FROM EMP GROUP BY MGR);
...
28SELECT Ename,Sal,Commission,DeptNo,AGE(CURRENT_DATE,Hire_Dtae) AS Expierence
     FROM Emp_Table
     WHERE EmpNo=(SELECT Mrg
                  FROM Emp_Table
                  GROUP BY Mgr);
